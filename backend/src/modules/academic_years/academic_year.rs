...
use crate::{
    ...
    modules::academic_years::services,
...
pub async fn get_all(db: web::Data<Database>, query: web::Query<PaginationParams>) -> HttpResponse {
    match services::get_all(db.get_connection(), query.into_inner()).await {
...
pub async fn get_by_id(db: web::Data<Database>, path: web::Path<u64>) -> HttpResponse {
    match services::get_by_id(db.get_connection(), path.into_inner()).await {
        Ok(data) => HttpResponse::Ok().json(ApiResponse::success(data)),
...
pub async fn create(
    ...
    match services::create(db.get_connection(), body.into_inner()).await {
        Ok(data) => HttpResponse::Ok().json(ApiResponse::success(data)),
...
pub async fn update(
    ...
    match services::update(db.get_connection(), path.into_inner(), body.into_inner())
        .await
    {
        Ok(data) => HttpResponse::Ok().json(ApiResponse::success(data)),
        Err(e) => HttpResponse::BadRequest().json(ApiResponse::<()>::error(e)),
...
pub async fn delete(db: web::Data<Database>, path: web::Path<u64>) -> HttpResponse {
    match services::delete(db.get_connection(), path.into_inner()).await {
