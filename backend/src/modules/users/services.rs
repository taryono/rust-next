...
pub async fn get_user_by_id(db: &DatabaseConnection, user_id: u64) -> Result<UserResponse, DbErr> {
...
/// Soft delete user
pub async fn soft_delete(db: &DatabaseConnection, user_id: u64) -> Result<(), DbErr> {
    let user = User::find_by_id(user_id)
        .one(db)
        .await?
        .ok_or_else(|| DbErr::RecordNotFound("User not found".to_string()))?;
...
pub async fn restore(db: &DatabaseConnection, user_id: u64) -> Result<(), DbErr> {
    let user = User::find_by_id(user_id)
        .one(db)
        .await?
        .ok_or_else(|| DbErr::RecordNotFound("User not found".to_string()))?;
...
pub async fn force_delete(db: &DatabaseConnection, user_id: u64) -> Result<(), DbErr> {
