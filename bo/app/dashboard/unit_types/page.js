...
export default function UnitTypes() {
  ...
  const {
    ...
    updateFilters,
    refresh,
  } = usePagination(api.getUnitTypes);
  ...
  const fetchUnitTypes = async () => {
    try { 
      ...
      alertError('Failed to fetch unit_types');
    } finally { 
    }
  };
  ...
  if (loading && unit_types.length === 0) {
    return (
      <AuthLayout>
      ...
      <div className="page">
        <div className="page-wrapper"> 
            ...
            <div className="container-xl">
              <div className="row g-2 align-items-center">
                <div className="col">
                  ...
                  <h2 className="page-title">Management Unit</h2>
                </div>
                
                <AddButton 
                  title="Add new unit type" 
                  onClick={() => openModal('add-unit-type', null, refresh)}
                />
              </div>
            </div>
          </div>
          ...
          <div className="page-body">
            <div className="container-xl">
              <div className="card">
                <CardHeader title={"Unit Type List"} viewMode={viewMode} onViewModeChange={setViewMode} />
                  ...
                  <TableFilters
                    ...
                    searchPlaceholder="Search unit_types..."
                  />
                      ...
                      <div className="card-body">
                        <div className="row row-cards">
                          {unit_types.map((unit, index) => (
                            <div key={unit_type.id} className="col-md-6 col-lg-4">
                              <div className="card card-sm">
                                <div className="card-body">
                                  <div className="d-flex align-items-center mb-3">
                                    <span className={`avatar avatar-lg rounded me-3 ${getAvatarColor(index)}`}>
                                      {getInitials(unit_type.code)}
                                    </span>
                                    <div className="flex-fill"> 
                                      <div className="font-weight-medium">{unit_type.name}</div>
                                      <div className="text-secondary small">{unit_type.description}</div>
                                    </div>
                                  </div> 
                                </div>
                              </div>
                      ...
                      <div className="table-responsive">
                        <table className="table table-vcenter card-table table-striped">
                          ...
                          <tbody>
                            {unit_types.map((unit, index) => (
                              <tr key={unit_type.id}>
                                <td className="text-secondary">{unit_type.code}</td>
                                <td>
                                  <div className="d-flex py-1 align-items-center">
                                    <span className={`avatar avatar-sm me-2 ${getAvatarColor(index)}`}>
                                      {getInitials(unit_type.name)}
                                    </span>
                                    <div className="flex-fill">
                                      <div className="font-weight-medium">{unit_type.name}</div>
                                    </div>
                                ...
                                <td className="text-secondary">{unit_type.description}</td>
