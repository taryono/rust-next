// api/src/modules/institution/sport/service.rs
// ============================================================================
// service.rs - Business Logic Only
// ============================================================================
use super::dto::{
    CreateSportRequest, SportResponse, UpdateSportRequest,
};
use super::repository::SportRepository;
...
impl SportService {
    pub fn new(repository: SportRepository) -> Self {
        Self { repository }
    }
    ...
    pub async fn create(
        ...
        if let Some(_) = self.repository.find_by_name(&request.name).await? {
            return Err(AppError::ConflictError(
                "Sport with this name already exists".to_string(),
            ));
        }

        // Parse start_date and end_date to NaiveDate
        // Build entity with parsed dates
        let active_model = sports::ActiveModel {
        ...
        Ok(SportResponse::from(created))
    }

    /// Get class by ID
    pub async fn get_by_id(&self, id: i64) -> Result<SportResponse, AppError> {
        let sports = self
            .repository
            .find_by_id(id)
            .await?
            .ok_or_else(|| AppError::not_found("Sport not found".to_string()))?;

        Ok(SportResponse::from(sports))
    }
    ...
    pub async fn get_all(
    ...
    ) -> Result<PaginatedResponse<SportResponse>, AppError> {
        ...
        let responses: Vec<SportResponse> = items
            .into_iter()
    ...
    pub async fn update(
        ...
        // Build update model
        let mut active_model = sports::ActiveModel {
            id: Set(id),
            updated_at: Set(chrono::Utc::now()),
            ..Default::default()
        };
