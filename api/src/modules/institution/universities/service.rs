// ============================================================================
// api/src/modules/universities/service.rs
// service.rs - Business Logic Only
// ============================================================================
use super::dto::{
    CreateUniversityRequest, UniversityResponse, UpdateUniversityRequest,
};
use super::repository::UniversityRepository;
use crate::errors::AppError;
use crate::utils::pagination::{PaginatedResponse, PaginationParams};
use entity::universities;
use sea_orm::Set;
...
impl UniversityService {
    pub fn new(repository: UniversityRepository) -> Self {
        Self { repository }
    }
    ...
    pub async fn create(
    ...
    ) -> Result<UniversityResponse, AppError> {
        // Validate request
        request
            .validate()
        ...
        if let Some(_) = self.repository.find_by_name(&request.name).await? {
            return Err(AppError::ConflictError(
                "University with this name already exists".to_string(),
            ));
        }

        // Parse start_date and end_date to NaiveDate
        // Build entity with parsed dates
        let active_model = universities::ActiveModel {
        ...
        Ok(UniversityResponse::from(created))
    }

    /// Get class by ID
    pub async fn get_by_id(&self, id: i64) -> Result<UniversityResponse, AppError> {
        let universities = self
            .repository
            .find_by_id(id)
            .await?
            .ok_or_else(|| AppError::not_found("University not found".to_string()))?;

        Ok(UniversityResponse::from(universities))
    }
    ...
    pub async fn get_all(
    ...
    ) -> Result<PaginatedResponse<UniversityResponse>, AppError> {
        ...
        let responses: Vec<UniversityResponse> = items
            .into_iter()
    ...
    pub async fn update(
        ...
        // Build update model
        let mut active_model = universities::ActiveModel {
            id: Set(id),
            updated_at: Set(chrono::Utc::now()),
            ..Default::default()
        };
        ...
        Ok(UniversityResponse::from(updated))
    }
