...
// MAIN COMPONENT
// =====================================================
export default function AddUnitModal({ data, onClose, onSuccess }) {
  const [loading, setLoading] = useState(false);
  const [foundations, setUnits] = useState([]);
  const isEdit = !!data;
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      const response = isEdit
        ? await api.updateUnit(data.id, payload)
        : await api.createUnit(payload);

      alertSuccess(isEdit ? 'Unit berhasil diperbarui' : 'Unit berhasil ditambahkan');
      onSuccess?.(response);
      onClose();
    } catch (err) {
      const errorMessage =
  ...
  const preparePayload = (formData) => {
    ...
    const optionalFields = ['code', 'unit_type', 'address', 'description', 'city', 'province', 'phone', 'email'];
          ...
          <FormSection title="Informasi Utama" icon="bi bi-building">
            <div className="row">
              <div className="col-md-8">
                <FormInput label="Nama Unit" error={errors.name} required>
                  <input
            ...
            <div className="row">
              <div className="col-md-6">
                <FormInput label="Tipe Unit" error={errors.unit_type}>
                  <select
                    className={`form-select ${errors.unit_type ? 'is-invalid' : ''}`}
                    {...register('unit_type')}
                  >
